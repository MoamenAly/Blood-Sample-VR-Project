    ...
    public class RemoteGrabber : MonoBehaviour {
        ...
        public virtual void Update() {
            if (PhysicsCheckType == RemoteGrabType.Raycast) {
                ...
                RaycastHit hit;
                if (Physics.SphereCast(transform.position, SphereCastRadius, transform.forward, out hit, SphereCastLength, RemoteGrabLayers)) {

                    //if (useCustomCollision)
                    //{
                    //    var hits = Physics.SphereCastAll(transform.position, SphereCastRadius, transform.forward, SphereCastLength, RemoteGrabLayers);

                    //    if (FindNearestDirection(hits, out RaycastHit nearestHit))
                    //    {
                    //        ObjectHit(nearestHit.collider);
                    //    }
                    //    else
                    //    {
        ...
        [SerializeField] bool useCustomCollision = false;
        [Button]
        void TestCase(){
         useCustomCollision = !useCustomCollision;
        }

        bool FindNearestDirection(RaycastHit[] array, out RaycastHit ray)
        {
            ray = default; // Initialize the out parameter
            if (array == null || array.Length == 0)
            {
                return false; // No elements in the array, return false
            }

            float minDistance = float.MaxValue; // Use float.MaxValue to initialize the minimum distance
            bool found = false; // Track if we found any valid hit

            for (int i = 0; i < array.Length; i++)
            {
                float dis = Vector3.Dot(transform.forward, (array[i].transform.position- transform.position).normalized);
                Debug.Log(dis);
                if (dis < minDistance)
                {
                    minDistance = dis; // Update the minimum distance
                    ray = array[i]; // Update the closest hit
                    found = true; // Set found to true
                }
            }

            return found; // Return whether we found a nearest hit
        }
