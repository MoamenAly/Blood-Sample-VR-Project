...
namespace BNG {
    public class GrabbableBezierLine : GrabbableEvents {
        ...
        private void LateUpdate() {
            ...
            if (lineToGrabber != null)
            {
                if (lineToGrabber.HeldGrabbable || lineToGrabber.RemoteGrabbingItem )
                {
                    LineToDraw.enabled = false;
                }
            }

            if (lineRemoteGrabbing != null) {
                if (lineRemoteGrabbing.RemoteGrabbingItem || lineRemoteGrabbing.HeldGrabbable)
                {
                    LineToDraw.enabled = false;
                }
            }
        ...
        public void DrawBezierCurve(Vector3 point0, Vector3 point1, Vector3 point2, LineRenderer lineRenderer) {
            ...
            float distance = Vector3.Distance(point0, point2);

            //var segment = distance * 60;
            var calculated = 0.5f * (distance / 1.5f);
            HeightAdjustment = calculated > 0.05f?calculated:0.05f;           
            lineRenderer.enabled = distance < 0.20 ? false : true;
